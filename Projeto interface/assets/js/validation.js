...
export function initFormValidation(){
  ...
  const draftBtn = document.getElementById('salvarRascunho');

  // Restaura rascunho
  const draft = loadDraft();
  if(draft){ Object.entries(draft).forEach(([k,v]) => { const el = form.elements.namedItem(k); if(el) el.value = v; }); }
  ...
  function validate(){
    ...
    // Consistência entre campos (cross-field)
    const hoje = new Date();
    const nasc = data.nascimento ? new Date(data.nascimento) : null;
    if(!nasc || isNaN(nasc)) msgs.push('Data de nascimento é obrigatória.');
    if(nasc && (hoje.getFullYear() - nasc.getFullYear() < 18 || (hoje.getFullYear()-nasc.getFullYear()===18 && (new Date(hoje.getFullYear(), nasc.getMonth(), nasc.getDate()) > hoje)))){
      msgs.push('Você deve ter 18 anos ou mais.');
    }
    ...
    if(!data.plano) msgs.push('Selecione um plano.');
    if(!form.termos.checked) msgs.push('Você deve aceitar os termos de uso.');
  ...
  form.addEventListener('submit', (e)=>{
    ...
    clearDraft();
    showErrors([]);
    okMsg.textContent = 'Formulário enviado com sucesso!';
  ...
  draftBtn?.addEventListener('click', ()=>{
    const { data } = validate();
    saveDraft(data);
    okMsg.textContent = 'Rascunho salvo (local).';
  });

  // Validação em tempo real (acessível)
  form.addEventListener('input', ()=>{
    const { ok, msgs } = validate();
    if(ok){ showErrors([]); } else { showErrors(msgs); }
  });
}
